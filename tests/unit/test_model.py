...
class TestLocation:
    ...
    def test_init(self, test_db):
        loc = model.Location(test_db.session, name=get_random_string(10),
                             order_id=get_random_int(3), id=get_random_int(3))
        sec = model.Section(name=get_random_string(10),
                            order_id=get_random_int(3), location_id=loc.id)
        test_db.session.add_all([loc, sec])
        test_db.session.commit()

        l_with_s = model.Location(test_db.session, name=loc.name,
                                  order_id=loc.order_id, id=loc.id)
        assert sec.id == l_with_s.sections[0].id
    ...
    def test_section_is_appended(self, test_db):
        ...
        sec = model.Section(name=get_random_string(10),
                            order_id=get_random_int(3), location_id=loc.id)
        test_db.session.add(sec)
        test_db.session.commit()

        loc_added = test_db.session.query(Location).filter_by(
            id=loc.id).first()
        assert sec in loc_added.sections
    ...
    def test_update_order_id(self, test_db):
        ...
        loc_updated = test_db.session.query(Location).filter_by(
            name=loc.name).first()
        assert loc_updated.order_id == new_order_id

    def test_location_has_shopping_list_item(self, test_db):
        loc = model.Location(test_db.session, name=get_random_string(10),
                             order_id=get_random_int(3))
        with mock.patch("hotshopper.model.Section") as SectionMock:
            s = SectionMock(location_id=loc.id)
            s.has_shopping_list_items.return_value = True

            loc.sections = [s]
            assert loc.has_shopping_list_items(1) is True

    def test_location_has_no_shopping_list_item(self, test_db):
        loc = model.Location(test_db.session, name=get_random_string(10),
                             order_id=get_random_int(3))
        with mock.patch("hotshopper.model.Section") as SectionMock:
            sec = SectionMock(location_id=loc.id)
            sec.has_shopping_list_items.return_value = False

            loc.sections = [sec]
            assert loc.has_shopping_list_items(1) is False
    ...
    def test_location_has_shopping_list_item_no_mocks(self, test_db):
        ...
        sec = model.Section(name=get_random_string(10),
                            order_id=get_random_int(1), location=loc)
        ing = model.Ingredient(name=get_random_string(10),
                               always_on_list=1, section_id=sec.id)
        sec.ingredients = [ing]
        test_db.session.add_all([loc, sec, ing])
        assert loc.has_shopping_list_items(1) is True
    ...
    def test_location_does_not_have_shopping_list_item_no_mocks(self, test_db):
        ...
        sec = model.Section(name=get_random_string(10),
                            order_id=get_random_int(1), location=loc)
        ing = model.Ingredient(name=get_random_string(10),
                               always_on_list=0, section_id=sec.id)
        sec.ingredients = [ing]
        test_db.session.add_all([loc, sec, ing])
        assert loc.has_shopping_list_items(1) is False
...
class TestRecipe:
    ...
    def test_add_new_recipe(self, test_db):
        rec = model.Recipe(id=1, name="TestRecipe")
        rec.add(test_db.session)
        rec_added = test_db.session.query(Recipe).filter_by(
            name="TestRecipe").first()
        assert rec.name == rec_added.name

    def test_add_duplicate_recipe(self, test_db):
        rec = model.Recipe(id=1, name="TestRecipe")
        rec.add(test_db.session)
        rec2 = model.Recipe(id=2, name="TestRecipe")
        with pytest.raises(DuplicateRecipeError):
            rec2.add(test_db.session)
    ...
    def test_recipe_select(self, test_db):
        ...
        selected_weeks = [1, 3]
        assert rec.selected
        assert rec.weeks == selected_weeks
    ...
    def test_add_ingredient_to_recipe(self, test_db):
        ...
        assert len(result) == 0
        ing = model.Ingredient(id=1, name="∞")
        rec_ing = model.RecipeIngredient(ingredient_id=rec.id,
                                         recipe_id=ing.id,
                                         quantity_per_person=100,
                                         unit=Unit.GRAM)
        rec.add_ingredient(rec_ing, test_db.session)
        test_db.session.commit()
    ...
    def test_remove_ingredient(self, test_db):
        rec = model.Recipe(id=1, name="TestRecipe")
        ing = model.Ingredient(id=1, name="TestIngredient")
        rec_ing = model.RecipeIngredient(ingredient_id=rec.id,
                                         recipe_id=ing.id,
                                         quantity_per_person=100,
                                         unit=Unit.GRAM)
        rec.add_ingredient(rec_ing, test_db.session)
        rec_ing.delete(test_db.session)
        result = test_db.session.query(RecipeIngredient).filter_by(
            ingredient_id=1).all()
        assert len(result) == 0
...
class TestRecipeIngredient:
    ...
    def test_change_ingredient_quantity_per_person(self, test_db):
        ...
        rec_ing = model.RecipeIngredient(ingredient_id=rec.id,
                                         ...
                                         unit=Unit.GRAM)
        rec.add_ingredient(rec_ing, test_db.session)
        rec_ing.update(test_db.session, quantity_per_person=2, unit=Unit.PIECE)
        result = test_db.session.query(RecipeIngredient).filter_by(
        ...
        for value in illegal_quantities:
            with pytest.raises(ValueError):
                rec_ing.update(test_db.session, quantity_per_person=value)
