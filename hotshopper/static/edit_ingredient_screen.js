...
function init() {
    ...
    SELECTED_LOCATION_ID = parseInt(document.getElementById("selected_location_id").childNodes[0].nodeValue);
...
// TODO: Copied from confirm_add_ingredient() -> needs adaptations

function confirm_edit_ingredient(edit = false) {
    if (!formcheck()) {
        return false;
    }
    ...
    let always_on_list;  // what is this for?
    ...
    const template_edit = (location_index) => `/confirm_edit_ingredient/${ingredient_id}_${location_index}_${section_index}_${non_food}`;
